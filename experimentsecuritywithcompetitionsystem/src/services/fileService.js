...
module.exports.getFileData = (userId, pageNumber, search) => {
        ...
        FROM file  WHERE created_by_id=${SqlString.escape(userId)}  LIMIT ${SqlString.escape(limit)} OFFSET ${SqlString.escape(offset)};
        SET @total_records =(SELECT count(file_id) FROM file WHERE created_by_id= ${userId}   );SELECT @total_records total_records; `;
        } else {
            designFileDataQuery = `SELECT file_id,cloudinary_url,design_title,design_description 
            FROM file  WHERE created_by_id=${SqlString.escape(userId)} AND design_title LIKE '%${SqlString.escape(search)}%'  LIMIT ${SqlString.escape(limit)} 
            OFFSET ${SqlString.escape(offset)};
            SET @total_records =(SELECT count(file_id) FROM file WHERE created_by_id= ${SqlString.escape(userId)} AND design_title LIKE '%${SqlString.escape(search)}%' );SELECT @total_records total_records;`;
        }
        //--------------------------------------------------------------------
